...
#[tauri::command]
pub fn update_settings(settings: Settings, app: AppHandle, state: State<'_, AppState>) -> Result<AppData, String> {
    let sanitized_settings = sanitize_settings(settings);
    let previous_shortcut = state.data.lock()
    ...
    let valid_list_ids: HashSet<String> = guard.settings.lists.iter().map(|l| l.id.clone()).collect();
    let valid_label_ids: HashSet<String> = guard.settings.labels.iter().map(|l| l.id.clone()).collect();
...
#[tauri::command]
pub fn set_global_shortcut(shortcut: String, app: AppHandle, state: State<'_, AppState>) -> Result<AppData, String> {
...
#[tauri::command]
pub fn set_autostart_enabled(enabled: bool, app: AppHandle, state: State<'_, AppState>) -> Result<AppData, String> {
